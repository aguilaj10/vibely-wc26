...
@Singleton
internal class OwnershipRepositoryImpl
    @Inject
    constructor(
        private val dao: OwnershipDao,
    ) : OwnershipRepository {
        override fun observeAll(): Flow<Map<String, Int>> = dao.observeAll().map { rows -> rows.associate { it.stickerId to it.quantity } }

        override fun observeQuantity(stickerId: String): Flow<Int> = dao.observeQuantity(stickerId).map { it ?: 0 }

        override suspend fun getQuantity(stickerId: String): Int = dao.getQuantity(stickerId) ?: 0
        ...
        override suspend fun importAll(quantities: Map<String, Int>) {
